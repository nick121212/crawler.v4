export default {
    ...
    "msgFlow": [{
        ...
        "partten": "role:crawler.plugin.transform,cmd:single",
        "jsonata": ["${'data': $substringAfter($$.queueItem.responseBody,'WB_detail').$substringBefore('大数据tag')}"],
        "data": {
            "expression": "$.{'queueItem':{'responseBody': $$.$trim().$replace('</script>','').$replace('</html>','').$replace('\\\\r\\\\n','').$replace('\\\\','').$replace('\\\"','\"')}}"
        },
        "result": "$.result"
    }, {
                ...
                "fields": {
                    "none": {
                        "data": [{
                            ...
                            "key": "videos",
                            "title": "文章详情中的所有图片",
                            "selector": [".media_box .WB_video"],
                            "htmlStrategy": "jsdom",
                            "dealStrategy": "array",
                            "data": [{
                                "methodInfo": { "attr": ["action-data"] },
                                "htmlStrategy": "jsdom",
                                "dealStrategy": "normal"
                            }]
                        }]
